...
export default function Navbar({ lang, t }: NavbarProps) {
  ...
  // ✅ FIXED isActive: Handle locale-prefixed paths properly
  ...
  const menuItems = [
    { href: path('/'), label: t?.nav?.home || 'Home' },
    { href: path('/about'), label: t?.nav?.about || 'About' },
    { href: path('/services'), label: t?.nav?.services || 'Services' },
    { href: path('/portfolio'), label: t?.nav?.portfolio || 'Portfolio' },
  ];
